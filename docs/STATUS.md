...
That implementation is not yet a supported public product. No public
`v0.3.2` tag or anonymously pullable core image exists. The public documentation
and installation flow have not been proven solely from public bytes.
...
The current working branch contains later deployment/CI/documentation changes
after `7636bf1`; it must be frozen to a new exact SHA and pass the complete gate
before tagging. `/api/build-info` and OCI labels, not the health endpoint alone,
...
Private Gitea remains the development ledger. Initial GitHub repositories will
be reviewed release snapshots rather than mirrors of private operational
history.
...
- a passing public CI, secret and image gate on one final `0.3.2` SHA (the clean
  exported-source browser and source gates now pass locally);
- public source snapshots, legal checks and anonymous image pulls;
...
- a public installation bundle and Devkit clean-room flow using only public
  downloads;
